...
export default function ProductsWithCategoryPage({
    ...
    const { data: products, isLoading: isProductLoading } = useQuery({
        queryKey: [
            "fetchProducts",
            activeSubInfo?._id == "all" ? "" : activeSubInfo?._id,
            supplier?._id,
        ],
        queryFn: ({ signal }) =>
            fetchProducts(signal, {
                ...
                page_size: 15,
                category:
                    activeSubInfo?._id == "all" ? undefined : activeSubInfo?._id,
                supplier: supplier?._id,
            }),
    });

    if (supplierLoading || categoryLoading || isProductLoading)
        return (
    ...
    return (
        <section className="h-[calc(100vh-64px)] flex flex-col md:max-w-7xl mx-auto md:px-4 py-4 md:py-6 ">
            {/* 🧭 Breadcrumb */}
            <nav className="flex items-center text-xs md:text-sm text-gray-600 mb-3 flex-wrap px-3 sm:px-6 pt-4">
            ...
            <div className="flex flex-1 overflow-hidden">
                {/* 🧭 Fixed Sidebar */}
                <aside className="border-r w-[80px] sm:w-[200px] px-2 sm:px-4 py-3 sm:py-4 flex-shrink-0">
                    ...
                    <div className="flex flex-col gap-2 sm:space-y-1">
                        ...
                        <button
                            ...
                            className={`flex sm:flex-row flex-col items-center sm:items-start text-center sm:text-left px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg text-[10px] sm:text-sm transition w-full ${activeSub === "all"
                        ...
                        {subCategories.map((sub: any) => (
                            <button
                                ...
                                className={`flex sm:flex-row flex-col items-center sm:items-start text-center sm:text-left px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg text-[10px] sm:text-sm transition w-full ${activeSub === sub.handle
                                    ? "bg-primary/10 text-primary font-medium"
                                    : "hover:bg-gray-50 text-gray-700"
                                    }`}
                            >
